...
void FileSystem::addRecord(const string& parentKey, NodeRecord<string>* record) {
    if (!treeList.addRecord(parentKey, record)) cerr << "Parent directory " << parentKey << " not found or is not a directory." << endl;

}

void FileSystem::deleteRecord(const string& key) {
    if (!treeList.removeRecord(key)) cerr << "Record " << key << " not found" << endl;
}

void FileSystem::deleteTree(const string& rootKey) {
    if (!treeList.removeRecord(rootKey)) cerr << "Tree not found" << endl;
}
